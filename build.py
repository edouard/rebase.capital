...
# Short content hash, so a change to site.js is not hidden by a cached copy.
SITE_JS_VERSION = hashlib.sha1(
    io.open(os.path.join(ROOT, "site.js"), "rb").read()
).hexdigest()[:8]
    ...
    <main>
      <section class="pt-14 pb-14 sm:pt-20 sm:pb-20">
        ...
        <div class="rise mt-12 grid gap-10 sm:mt-16 sm:grid-cols-[13rem_1fr] sm:gap-12" style="animation-delay:0.12s">
          <img src="/images/edouard.jpg" alt="Edouard Bri&egrave;re" width="800" height="800"
...
POSTS = [
    ...
    {
        "slug": "2007-2012-building-it",
        "old_slug": "act-1-the-technical-war",
        "series": "eighteen-years",
        "part": 1,
        "title": "2007–2012: Building it",
        "date": "2026-07-25",
        "date_label": "25 July 2026",
        "summary": "The first of three pieces about eighteen years of WebTranslateIt — what it taught me about products, markets, and why I built my second company as the structural opposite of my first.",
    },
    {
        "slug": "2012-2024-when-the-money-arrived",
        "old_slug": "act-2-the-capital-war",
        "series": "eighteen-years",
        "part": 2,
        "title": "2012–2024: When the money arrived",
        "date": "2026-08-20",
        "date_label": "20 August 2026",
        "summary": "What happened when the money arrived: paying Google for the right to be found by people already searching for my own product, competing against balance sheets instead of products, and the year I stopped fighting for growth.",
    },
    {
        "slug": "2024-2026-starting-again",
        "old_slug": "act-3-the-settlement",
        "series": "eighteen-years",
        "part": 3,
        "title": "2024–2026: Starting again",
        "date": "2026-09-10",
        "date_label": "10 September 2026",
        "summary": "Writing a specification against everything the first eighteen years cost, building a second company on the one channel that cannot be outbid, and the honest accounting of what actually decided the old defeat.",
    },
    {
        "slug": "an-ai-sre-for-a-one-person-company",
        "title": "Why I’m building an AI SRE for a one-person company",
        "date": "2026-09-18",
        "date_label": "18 September 2026",
        "summary": "I’ve been the entire on-call rotation since 2008. I’m building an AI agent to take some of that work off my plate.",
    },
    {
        "slug": "distill-a-shared-inbox-that-writes-a-wiki",
        "title": "Distill: a shared inbox that writes a wiki",
        "date": "2026-09-19",
        "date_label": "19 September 2026",
        "summary": "What I know about a customer is scattered across emails, chats and screenshots. Distill is the internal tool I built to read them and keep the notes I never would.",
    },
]
...
def series_nav(post):
    """Previous/next links between the parts of a multi-post series."""
    if "series" not in post:
        return ""
    parts = sorted((p for p in POSTS if p.get("series") == post["series"]), key=lambda p: p["part"])
    i = parts.index(post)
    prev = parts[i - 1] if i > 0 else None
    nxt = parts[i + 1] if i + 1 < len(parts) else None
    links = []
    if prev:
        links.append(f'<a href="/blog/{prev["slug"]}/">&larr; {prev["title"]}</a>')
    if nxt:
        links.append(f'<a href="/blog/{nxt["slug"]}/">{nxt["title"]} &rarr;</a>')
    return (
        '          <p class="kicker">\n'
        '            <em>In this series</em><br>\n'
        + "".join(f"            {l}<br>\n" for l in links)
        + '          </p>'
    )
...
AIOPS_BODY = """
          <p>
            ...
            moving. A server that wants a reboot. The alert arrives, I open a terminal, and for
            the next twenty minutes I am a systems administrator with no colleagues and no
            handover notes except the ones I wrote myself, years ago, in a wiki with pages called
            <em>Queue overload mitigation</em> and <em>Recovering software RAID</em>.
          </p>
          <p>
            <a href="https://webtranslateit.com" target="_blank" rel="noopener">WebTranslateIt</a>
            has been in production since 2008. These days I also run
            <a href="https://trackberry.com" target="_blank" rel="noopener">Trackberry</a>, and
            <a href="/blog/distill-a-shared-inbox-that-writes-a-wiki/">Distill</a>, an internal tool
            I&rsquo;ve since written about. That makes three Rails apps on three
            servers at two hosting providers, looked after by a team the same size it was in
            2008: me.
          </p>
          <p>
            I want something to take part of that work off my plate. I thought about it a lot over
            the summer, and started building it this week. So treat this as a build log, not a
            success story.
          </p>
          <h2>Ops with a team of one</h2>
          <p>
            The monitoring is fine. I run a self-hosted <a href="https://signoz.io" target="_blank"
            rel="noopener">SigNoz</a> and the alerts fire when they
            should. But every alert still ends with me opening a terminal.
          </p>
          <p>
            Outages aren&rsquo;t actually the expensive part; they&rsquo;re rare. It&rsquo;s the
            little things that keep stealing my attention: the reboot a kernel update is waiting
            on, the SigNoz upgrade I do by hand every month. Each one is ten minutes, which is
            exactly why I&rsquo;ve never automated any of them.
          </p>
          <h2>Why not more scripts</h2>
          <p>
            Scripts handle the things I already know how to handle. The incidents that cost me time need a little
            judgment. Is this disk growth normal? Which service do I restart first?
          </p>
          <p>
            I&rsquo;ve tried the obvious solution: hire someone who can share the on-call. I&rsquo;ve never found a developer who wanted the job. I&rsquo;ve been doing this for eighteen years, and I can hardly blame them.
            Being woken up to investigate a broken background job on someone else&rsquo;s
            infrastructure isn&rsquo;t exactly why most people become developers. Managed ops
            makes little sense for three servers either.
          </p>
          <p>
            So I want something in between: something that reads the runbook, checks the signals,
            does the investigation, and eventually carries out a few repairs within limits I set.
          </p>
          <h2>What I&rsquo;m building</h2>
          <p>
            It lives in one repository: checks, runbooks, rules and agent code together. I want to
            be exact about what exists, because posts like this one usually aren&rsquo;t.
          </p>
<pre><code>systemd timer, every 5 min
      |
   watcher  -- read-only probe over SSH --&gt;  the three apps
    ...
    agent   -- rules, runbook, SSH output --&gt;  Claude
      ...
      v
    Slack   -- later: approve --&gt;  allowlisted action</code></pre>
          <p>
            <strong>Running today: the watcher.</strong> A systemd timer on a box I already operate
            runs one check per app every five minutes. To reach the other hosts it uses an SSH key that each server restricts to a single read-only script. The key
            cannot open a shell. None of it runs in GitHub Actions, on purpose: scheduled workflows there are
            best-effort, and a watcher that silently skips ticks is worse than no watcher.
          </p>
          <p>
            A check never stops at the first failure. It gathers every signal it can and reports
            the healthy ones too, so a failure arrives in Slack as one line like this:
          </p>
<pre><code>wti: goodjob: worker heartbeat is 2840s stale (limit 120s)
  [ok: http 200 from webtranslateit.com/up;
       postgres active, 82/450 connections;
       goodjob 0 failures in the last hour]</code></pre>
          <p>
            The site is up, the database is fine, nothing is erroring, and the worker hasn&rsquo;t
            checked in for 47 minutes. That&rsquo;s enough context for me to know where I&rsquo;d start looking.
          </p>
          <p>
            The agent exists, but I don&rsquo;t trust it yet. It&rsquo;s a small Python program
            that takes that line, loads the rules and the app&rsquo;s runbook, and calls Claude
            with exactly one tool: run a command over SSH. The first version is read-only. The runbook has a row for this symptom: a heartbeat row that is present but stale means
            the worker may be hung, and restarting it is allowed with approval. The agent&rsquo;s
            job is to gather the evidence and decide whether that row really applies, or whether
            this is something else wearing the same symptom. What I want back is something like:
          </p>
<pre><code>Hypothesis   worker container is up but hung
...
Confidence   high</code></pre>
          <p>
            I wrote that report by hand. The agent hasn&rsquo;t diagnosed a real incident yet, and
            the first few weeks will be me comparing what it says with what I would have said.
          </p>
          <p>
            After that comes a short list of repairs: restart a service named in the runbook,
            clear listed temp directories, kill a runaway process that matches a known failure
            mode, retry a background job. Each one sits behind an approve button in Slack until I
            decide, action by action, to take the button away.
          </p>
          <p>
            The chores I opened with aren&rsquo;t on that list. Night reboots and the monthly
            upgrade are what I most want to stop doing, and for now both are forbidden: no
            rebooting a host, no installing packages. I&rsquo;d rather earn my way to them.
          </p>
          <h2>It has already found something</h2>
          <p>
            Before the agent has fixed anything, the project has found something I had missed,
            and it had nothing to do with AI. The first phase was just me filling in runbooks and
            checking them against the live hosts. Doing that, I found that WebTranslateIt&rsquo;s
            database backup had silently not run for three weeks. The container&rsquo;s cron
            daemon ignores a crontab that isn&rsquo;t owned by root, and the deploy tool mounted
            it as another user. When I looked in the storage bucket the nightly dumps are uploaded to, there was one
            file where there should have been about twenty. Nothing anywhere had said so.
          </p>
          <p>
            I fixed the ownership problem, and every backup now pings an outside service that complains when
            it goes quiet. I am precisely the kind of operator who needs a second pair of eyes.
          </p>
          <h2>Where the guardrails are</h2>
          <p>
            The runbooks are the contract. If it isn&rsquo;t written down, the agent doesn&rsquo;t
            do it; it escalates and never improvises. I care more about what it can&rsquo;t do
            than what it can. Nothing touches Postgres data, changes configuration, deploys code,
            or manages users and keys. And those aren&rsquo;t instructions to the model. The agent gets its own SSH account, separate from the watcher&rsquo;s key, and the
            account&rsquo;s <code>sudoers</code> file lists the exact commands it may run. A prompt is a request. A sudoers file is a fact.
          </p>
          <p>
            It hands over to me when the fix isn&rsquo;t on the list, when a fix didn&rsquo;t work,
            or when what it finds doesn&rsquo;t match what the runbook describes. For now its
            confidence also has to be high. Every run reports, including the ones
            that find nothing, so I can always read what happened while I slept.
          </p>
          <h2>What I expect to go wrong</h2>
          <p>
            I don&rsquo;t have to guess at all of it, because some has already happened.
          </p>
          <p>
            The first time the probe ran against production it reported a worker heartbeat 7,209
            seconds stale, against a limit of 120, on a healthy server. The job tables store UTC
            in a column with no timezone, that host&rsquo;s database session is set to Berlin, and
            every age came out exactly two hours too large. The other two hosts run on UTC, so the
            code was right on two machines out of three, which is the worse way to be wrong. My
            self-test didn&rsquo;t catch it; it feeds the checks fixed values, not SQL.
          </p>
          <p>
            So false positives aren&rsquo;t an edge case. They&rsquo;re going to happen. Right now
            every failing run posts, with no deduplication, because I want to see how noisy it actually is before I decide how to deal with it. If I start ignoring the Slack channel
            because there are too many false alarms, the whole thing has failed.
          </p>
          <p>
            What worries me more is an agent that finds a plausible explanation and then does the
            wrong thing. A model that says &ldquo;I don&rsquo;t know&rdquo; is cheap. One that
            reads a log, builds a plausible story and restarts the wrong thing is expensive, and
            asking it to rate its own confidence is only a partial defence. That&rsquo;s why
            version one can&rsquo;t act, version two needs a button, and Postgres is off the table
            entirely.
          </p>
          <p>
            And now I have another system to maintain. The watcher shares a host with Distill and
            SigNoz, so if I lose that one machine I lose an app, the alerting and the thing
            watching them, all together. I&rsquo;m betting it breaks less often than what it
            watches. I don&rsquo;t know yet whether that&rsquo;s true.
          </p>
          <h2>Beyond my servers</h2>
          <p>
            I stayed small on purpose, and I&rsquo;ve <a
            href="/blog/2024-2026-starting-again/">written at length</a> about how that happened.
            The trade-off has always been that there&rsquo;s a limit to how much I can operate
            myself. If an agent can take over some of the boring operational work without taking over the
            important decisions, I can keep more infrastructure running without adding another
            person to the rota. Maybe that changes what a company of one can operate. I
            don&rsquo;t know yet.
            That&rsquo;s what I&rsquo;m trying to find out, on my own servers, with eighteen years
            of runbooks and an agent that isn&rsquo;t allowed to touch the database.
          </p>
          <h2>What&rsquo;s next</h2>
          <p>
            Read-only diagnosis first, then the button. I&rsquo;ll write up what happens the first
            time it fixes something without me, and just as plainly if the first thing it does is
            get something wrong. If you&rsquo;re building something similar for a company of one
            or two, I&rsquo;d like to compare notes; I&rsquo;m on <a
            href="https://www.linkedin.com/in/edouard-bri%C3%A8re-150247150/" target="_blank"
            rel="noopener">LinkedIn</a>.
          </p>
          <p class="colophon">
            <em>M&aacute;laga, 18 September 2026.</em>
          </p>
"""

DISTILL_BODY = """
          <p>
            In <a href="/blog/an-ai-sre-for-a-one-person-company/">the last post</a> I mentioned
            Distill, an internal tool I said I&rsquo;d tell you more about. This is that post.
          </p>
          <p>
            I forward an email to it, paste in a
            WhatsApp chat, or drop in a screenshot or a contract. It reads what I gave it, works
            out who and what it is about, and writes what it learned onto pages about the people,
            companies and projects I deal with.
          </p>
          <h2>The notes I never write</h2>
          <p>
            Everything I know about a customer already exists somewhere. It&rsquo;s in an email
            thread from March, a chat on my phone, a photo of a whiteboard, a PDF somebody sent
            once. What doesn&rsquo;t exist is the page that says who this person is, what their
            company grows, what software they run, and what we agreed last time.
          </p>
          <p>
            Every CRM I&rsquo;ve used assumes I&rsquo;ll type that page myself after each call. I
            don&rsquo;t. Nobody does for long. But I will forward an email, because forwarding an
            email takes four seconds. So Distill asks for nothing more than that, and does the
            writing itself. It&rsquo;s a shared inbox that writes a wiki.
          </p>
          <h2>What a page looks like</h2>
          <p>
            A page is a markdown file. It opens with a sentence or two saying who this is, and
            then it&rsquo;s a list of facts:
          </p>
<pre><code>Runs procurement at Acme Cloud, where she has been our
...
- Based in Amsterdam since June [src:412 2026-08-14]</code></pre>
          <p>
            The thing at the end of each line is what makes this useful. It&rsquo;s a link to
            the source that said so: the email, the chat, the transcript of the screenshot. When an AI writes your notes, the question you&rsquo;ll ask
            of every sentence is &ldquo;says who?&rdquo;, and the answer has to be one click away.
          </p>
          <p>
            The struck-through line is a fact that stopped being true. It stays on the page with
            its own source, so I can see what changed and when without digging through history.
          </p>
          <h2>What stays out of the wiki</h2>
          <p>
            &ldquo;I&rsquo;ll send the quote on Friday&rdquo; is not a fact about anybody. Once
            Friday has passed it&rsquo;s either done or missed, and in six months it&rsquo;s worth
            nothing. So it goes on a calendar, not on a page. If it won&rsquo;t be useful in six months, it
            doesn&rsquo;t belong on the page.
          </p>
          <p>
            A reminder can simply be deleted. A sourced fact shouldn&rsquo;t disappear without
            leaving its history behind.
          </p>
          <h2>When it doesn&rsquo;t know, it asks</h2>
          <p>
            The most expensive mistake a tool like this can make is merging two people into one.
            So an email address or a domain is treated as evidence, and a similar name is treated
            as a coincidence. If a new &ldquo;Mike&rdquo; could plausibly be either of two Mikes
            I already know, Distill doesn&rsquo;t guess. It asks me, and only the facts about
            that Mike wait for the answer. Everything else in the email is written straight away.
          </p>
          <p>
            It does the same when a new fact contradicts an old one: it asks me rather than
            deciding which one is right. If I ignore a question
            for a week, it takes the conservative option: create a new page that I can merge
            later, or keep both facts.
          </p>
          <h2>Plain files in a Git repository</h2>
          <p>
            The wiki is a Git repository of markdown. If I clone it, I can read all of it in any
            markdown viewer with no application running, and the source links still work, because
            the text of every source is in the repository too. If Distill disappeared tomorrow I
            would still have the notes. After eighteen years of running software, that&rsquo;s the
            property I trust most.
          </p>
          <p>
            The attachments themselves live in a database, not in Git. Git is deliberately bad at
            forgetting things, and deleting somebody&rsquo;s data needs to actually delete it.
          </p>
          <h2>How it&rsquo;s put together</h2>
<pre><code>email, chat, screenshot, PDF
      |
   ...
   write     one page at a time               (Claude Sonnet)
      |
      v
   one Git commit per source</code></pre>
          <p>
            It&rsquo;s one Rails app. Each step is a small, separate call to Claude. There&rsquo;s
            a reason for each one. Decisions used to be one more field in the extraction step.
            The model then returned no decisions at all from a thread whose whole subject was two
            people agreeing terms, and the rest of the extraction got worse too. What was actually agreed is a different question from what was mentioned, so it gets
            its own pass.
          </p>
          <p>
            Extraction runs on the larger model for a similar reason. The small one filed
            companies under <code>people/</code> without hesitating, and every later step inherits
            that mistake.
          </p>
          <p>
            Nothing in processing a source grows with the wiki. Search picks a shortlist of pages, and those
            are the only ones the model sees, so a source touches two to six pages whether the
            wiki has a hundred of them or ten thousand. That keeps the bill flat, but the real reason is correctness: a model handed everything reads all of it badly.
          </p>
          <h2>Where the model isn&rsquo;t trusted</h2>
          <p>
            A model rewriting a whole page will occasionally drop a source link, and a page that
            lost its provenance is worse than a page that missed an update. So every rewrite is
            compared before and after, and refused if a link went missing.
          </p>
          <p>
            That check once passed a rewrite it shouldn&rsquo;t have. A company&rsquo;s logo
            vanished from its page: the model had kept the line and its source, and replaced the
            image with the words &ldquo;their wordmark&rdquo;. The same comparison now runs over
            images as well.
          </p>
          <p>
            There&rsquo;s also a nightly pass that tidies up contradictions which built up over
            months. It&rsquo;s never given a page to rewrite. It quotes lines back, and ordinary
            code finds those lines and does the editing. A line quoted inexactly matches nothing,
            and nothing happens.
          </p>
          <p>
            The general rule is simple: let the model interpret things, but don&rsquo;t let it
            silently rewrite state.
          </p>
          <h2>Where it stands</h2>
          <p>
            Distill is an internal tool, and I use it every day at
            <a href="https://trackberry.com"  target="_blank" rel="noopener">Trackberry</a>. A
            prospect&rsquo;s page opens with the handful of things I always want to know before a
            call: what they grow, how much, who matters there, and what systems they use. It shows the
            unanswered questions as plainly as the answered ones, which tells me what to ask next.
          </p>
          <p>
            It isn&rsquo;t a product and I don&rsquo;t know whether it should be one. There are
            still rough edges, and a few things I&rsquo;d like it to do that it doesn&rsquo;t yet.
            But for now it&rsquo;s the colleague who reads everything and writes it down, which is
            the colleague I&rsquo;ve been missing since 2008.
          </p>
          <p class="colophon">
            <em>M&aacute;laga, 19 September 2026.</em>
          </p>
"""

POSTS[0]["body"] = POST_BODY
POSTS[1]["body"] = ACT1_BODY
POSTS[2]["body"] = ACT2_BODY
POSTS[3]["body"] = ACT3_BODY
POSTS[4]["body"] = AIOPS_BODY
POSTS[5]["body"] = DISTILL_BODY


def write_redirect(old_slug, slug, title):
    """A stub at a post's previous address, pointing at where it lives now."""
    target = f"/blog/{slug}/"
    path = os.path.join(ROOT, "blog", old_slug, "index.html")
    os.makedirs(os.path.dirname(path), exist_ok=True)
    io.open(path, "w", encoding="utf-8").write(f"""<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <title>{title} &mdash; Rebase Capital</title>
...
<body>
  <p>This post has moved to <a href="{target}">{title}</a>.</p>
</body>
</html>
""")
    print(f"wrote blog/{old_slug}/index.html (redirect)")
...
def write_feed():
    ...
    for post in sorted(POSTS, key=lambda p: p["date"], reverse=True):
        ...
        # The id is the address the entry was first published under, so a later
        # rename moves the link without re-announcing the post to subscribers.
        entry_id = f"{SITE}/blog/{post.get('old_slug', post['slug'])}/"
...
if __name__ == "__main__":
    ...
    for post in POSTS:
        write(
            ...
            post["summary"],
            f"/blog/{post['slug']}/",
            post_main(post, post["body"]),
            current="Blog",
            og_type="article",
        )
    for post in POSTS:
        if "old_slug" in post:
            write_redirect(post["old_slug"], post["slug"], post["title"])
    write_feed()
